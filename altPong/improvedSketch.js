...
// paddle noises
var paddleMovingUp = false;
var paddleMovingDown = false; 
...
function preload() {
  ...
  lightOff = loadSound('lightOff.mp3');
	paddleMoving = loadSound('laser.mp3');
}
...
function draw() {
  ...
  if(isPlayingGame == 1){
    ...
    fill(255,255,255,ballColor);
    noStroke();
    rect(ballX, ballY, ballWidth, ballHeight);

    // paddles
		paddleMoving.setVolume(0.3);
		fill(255,255,255,paddleColor);
    if(revealPaddle){
      rect(paddle1X, paddle1Y, paddleWidth+10, paddleHeight+10);
			rect(paddle2X, paddle2Y, paddleWidth+10, paddleHeight+10);
			lightOn.play(); 
      revealPaddle = false; 
    }
    if(paddleColor > 0){
